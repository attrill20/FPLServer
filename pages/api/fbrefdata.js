...
export default async function handler(req, res) {
  ...
  try {
    ...
    $("table.stats_table tbody tr").each((index, element) => {
      ...
      const xg_diff = parseFloat($(element).find("td[data-stat='xg_diff']").text().trim().replace("+", "")) || 0;
      const xg_diff_per90 = parseFloat($(element).find("td.right.group_start[data-stat='xg_diff_per90']").text().trim().replace("+", "")) || 0;
